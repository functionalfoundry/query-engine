...
(deftest goto-attr
  (let [z (dz/data-zipper {:foo :bar :baz :ruux})]
    (and (is (= :bar (-> z (dz/goto-attr :foo) zip/node)))
         (is (= :ruux (-> z (dz/goto-attr :baz) zip/node))))))

(deftest goto-parent-map
  (let [m {:foo :bar :baz :ruux}
        z (dz/data-zipper m)]
    (and (is (= m (-> z (dz/goto-attr :foo)
                      dz/goto-parent-map zip/node)))
         (is (= m (-> z (dz/goto-attr :baz)
                      dz/goto-parent-map zip/node))))))

(deftest set-attr
  (let [m {:foo :bar :baz :ruux}
        z (dz/data-zipper m)]
    (and (is (= {:foo :FOO :baz :ruux}
                (-> z (dz/set-attr :foo :FOO) zip/node)))
         (is (= {:foo :bar :baz :BAZ}
                (-> z (dz/set-attr :baz :BAZ) zip/node)))
         (is (= {:foo :FOO :baz :BAZ}
                (-> z
                    (dz/set-attr :foo :FOO)
                    (dz/set-attr :baz :BAZ)
...
(deftest first-entity-and-next-entity
  (let [c [{:db/id 1} {:db/id 2} {:db/id 3}]
        z (dz/data-zipper c)]
    (and (is (= {:db/id 1} (-> z dz/first-entity zip/node)))
...
(deftest last-entity?
  (let [c [{:db/id 1} {:db/id 2} {:db/id 3}]
        z (dz/data-zipper c)]
    (and (is (false? (-> z dz/first-entity dz/last-entity?)))
